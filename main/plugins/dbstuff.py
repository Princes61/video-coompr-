...
async def bban(event):
    ...
    if not c:
        await event.reply("Who do ypu wanna ban!?")
    ...
    if xx is True:
        ...
        await event.reply(f"{c} now is banned.")
...
async def unbban(event):
    ...
    if not xx:
        await event.reply("Who do want to unban?")
    xy = await db.is_banned(int(xx))
    if xy is False:
        return await event.reply("User is already unbanned!")
    await db.unbanning(int(xx))
    await event.reply(f"{xx} Yep this user can use me! ")
