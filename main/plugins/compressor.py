...
async def compress(event, msg, ffmpeg_cmd=0, ps_name=None):
    ...
    if ffmpeg_cmd == 1:
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}""" -preset ultrafast -vcodec libx265 -crf 29 -acodec copy """{out}""" -y'
    elif ffmpeg_cmd == 2:
        ...
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}""" -c:v libx265 -crf 29 -preset ultrafast -s 640x360 -c:a copy """{out}""" -y'
    elif ffmpeg_cmd == 4:
    ...
    p_size = (100 - (f_size/i_size) * 100)
    p1_size = round(p_size, 2)
    text = F'**ENCODED by:** @{BOT_UN}'
    if ps_name != "**ENCODING:**":
        text = f'**COMPRESSED by** : @{BOT_UN}\n\nInitial size: `{round(i_size/1000000, 2)}`mb\n\nFinal size: `{round(f_size/1000000, 2)}`mb\n\nCompression Ratio: `{p1_size}`%'
    UT = time.time()
